...
/* ─── Design tokens — Parent = Orange ─── */
const ORANGE      = '#ea580c'; // orange-600
const ORANGE_DARK = '#9a3412'; // orange-800
const ORANGE_LITE = '#ffedd5'; // orange-100
const ORANGE_MID  = '#f97316'; // orange-500

const featureCards = [
  {
    icon: 'monitoring',
    title: 'Track Progress',
    desc: 'Monitor your child\'s grades, attendance, and academic performance.',
  },
  {
    icon: 'notifications_active',
    title: 'Stay Updated',
    desc: 'Receive real-time notifications about school events and announcements.',
  },
  {
    icon: 'forum',
    title: 'Communicate',
    desc: 'Easily message teachers and school administration.',
  },
];

const trustBadges = [
  { label: '2M+',    sublabel: 'Parents', icon: 'family_restroom' },
  { label: 'Secure', sublabel: 'Data', icon: 'verified_user' },
  { label: '100%',   sublabel: 'Free Tools', icon: 'redeem' },
];
  ...
  return (
    <>
      ...
      >
        ...
        <div className="flex-1 bg-white dark:bg-slate-900 flex flex-col px-10 py-10 lg:px-14 lg:py-12 overflow-y-auto transition-colors duration-300">
          ...
          <div className="flex items-center justify-between mb-8">
            <div className="flex items-center gap-2.5">
              ...
              >
                <span className="material-symbols-outlined text-white text-[17px]">family_restroom</span>
              </div>
              <span className="text-[15px] font-semibold" style={{ color: ORANGE }}>Qefas Hub</span>
            </div>
            <button
              type="button"
            ...
            >
              ...
              <span className="font-semibold" style={{ color: ORANGE }}>Sign in</span>
            </button>
          </div>
          ...
          <div className="mb-8">
            ...
            >
              Join Qefas Hub<br />
              <span style={{ color: ORANGE }}>as a Parent</span>
            </h1>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="flex-1 flex flex-col gap-4">
            ...
            <div className="border border-slate-200 dark:border-slate-700 rounded-xl overflow-hidden mt-2">
              ...
              <div className={`transition-all duration-300 ease-in-out bg-white dark:bg-slate-900 ${showOptional ? 'max-h-[500px] opacity-100 p-4 border-t border-slate-200 dark:border-slate-700' : 'max-h-0 opacity-0 pointer-events-none'}`}>
                <div className="grid grid-cols-1 gap-4">
                  <Field
                    label={<>Student Code <span className="font-normal text-slate-400 dark:text-slate-500">(Link your child)</span></>}
                    icon="badge"
            ...
            <div className="flex items-start gap-2.5 pt-2">
              ...
              <label htmlFor="parent-terms" className="text-[13px] text-slate-500 dark:text-slate-400 leading-relaxed cursor-pointer">
                I agree to the{' '}
            ...
            <button
              ...
              style={{ background: `linear-gradient(135deg, ${ORANGE_DARK} 0%, ${ORANGE} 100%)`, height: '52px' }}
            >
              {/* hover shimmer */}
        ...
        {/* ════════════════════════════════════
            RIGHT — VISUAL PANEL (orange gradient)
        ════════════════════════════════════ */}
        <div
          className="hidden lg:flex flex-col w-[44%] shrink-0 relative overflow-hidden"
          style={{ background: `linear-gradient(160deg, ${ORANGE_DARK} 0%, ${ORANGE} 60%, ${ORANGE_MID} 100%)` }}
        >
          {/* Ambient blobs */}
          <div
          ...
          <div className="relative z-10 flex items-center justify-between px-8 pt-8 pb-4">
            ...
            <div className="text-white/60 text-[12px]">Join thousands of parents</div>
          </div>

          {/* Hero image */}
          <div className="relative mx-6 rounded-2xl overflow-hidden flex-shrink-0" style={{ height: '320px' }}>
            <Image
              src="/images/messimo_style_3d_parent_orange.png"
              alt="Parent reviewing student progress"
              fill
            ...
            <div
              ...
              style={{ background: `linear-gradient(to top, ${ORANGE_DARK} 0%, transparent 60%)` }}
            />
            ...
            >
              <span className="w-2 h-2 rounded-full animate-pulse bg-orange-300" />
              <span className="text-white text-[13px] font-medium">Your parent portal</span>
            </div>
          </div>
                ...
                {featureCards.map((_, i) => (
                  <button
                    ...
                    style={{
                      ...
                      height: '6px',
                      background: i === activeFeature ? '#fdba74' : 'rgba(255,255,255,0.3)',
                    }}
                  />
                ))}
              </div>
          ...
          <div className="relative z-10 grid grid-cols-3 gap-3 mx-6 mt-5 mb-8">
            {trustBadges.map(b => (
              ...
              >
                <span className="material-symbols-outlined text-[18px] mb-1.5 block text-orange-300">
                  {b.icon}
                </span>
...
function inputCls(hasError: boolean) {
  return [
    ...
    hasError
      ...
      : 'border-slate-200 dark:border-slate-700 focus:ring-2 focus:ring-orange-500/30 focus:border-orange-500/50',
  ].join(' ');
}
  ...
  return (
    <div className="p-6 border-t border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/50">
      <button
        ...
        style={{ background: `linear-gradient(135deg, ${ORANGE_DARK} 0%, ${ORANGE} 100%)` }}
      >
        {label}
      </button>
    </div>
