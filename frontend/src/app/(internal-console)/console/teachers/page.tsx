...
export default function PlatformTeachersPage() {
    ...
    const { data: response, isLoading } = usePlatformTeachers(searchQuery, currentPage, 10, planFilter, statusFilter)
    ...
    const statusCards = [
        { title: "Paid Teachers", value: statusBreakdown.find((s: any) => s.type === 'PAID')?.count || 0, icon: CreditCard, color: "text-emerald-500", bg: "bg-emerald-500/10" },
        ...
        <div className="space-y-8 pb-20">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-[repeat(auto-fill,minmax(320px,1fr))] gap-6">
                {isLoading ? (
                    ...
                    [
                        { name: "Total Teachers", count: pagination?.total || 0, type: 'TOTAL' },
                        ...planBreakdown
                                ...
                                <div className="flex justify-between items-start">
                                    <div>
                                        <p className="text-[10px] font-bold uppercase tracking-[0.2em] opacity-80">
                                            {plan.type === 'TOTAL' ? "Staff Demographics" : "Subscription Plan"}
                                        </p>
                                ...
                                <div className="flex justify-between items-end">
                                    <div>
                                        <p className="text-[10px] font-medium opacity-60">
                                            {plan.type === 'TOTAL' ? "Total Faculty" : "Staff Enrolled"}
                                        </p>
                                        ...
                                        <td className="px-8 py-6">
                                            <div className="flex flex-col gap-2">
                                                <div className="flex items-center gap-2 text-sm font-bold text-slate-800 dark:text-slate-200">
                                                    <Link 
                                                        href={`/console/schools/${teacher.school?.id}`}
                                                        className="hover:text-indigo-500 flex items-center gap-1 transition-colors"
                                                    >
                                                        <SchoolIcon size={14} className="text-slate-400" />
                                                        {teacher.school?.name || "Independent"}
                                                        <ExternalLinkIcon size={10} className="opacity-0 group-hover:opacity-100 transition-opacity" />
                                                    </Link>
                                                </div>
                                                <div className="flex items-center gap-2">
                                                    <Badge className="bg-slate-500/10 text-slate-500 dark:text-slate-400 border-none px-2 py-0.5 rounded-full flex items-center gap-1 font-bold text-[9px] uppercase tracking-widest">
                                                        <BookOpenIcon size={10} />
                                                        {teacher._count?.classTeachers || 0} Classes
                                                    </Badge>
                                                    <Badge className={cn(
