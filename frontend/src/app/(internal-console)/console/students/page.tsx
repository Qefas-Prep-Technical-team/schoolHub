...
export default function PlatformStudentsPage() {
    ...
    const { data: response, isLoading } = usePlatformStudents(searchQuery, statusFilter, currentPage, 10, planFilter)
    ...
    const statusCards = [
        { title: "Paid Students", value: statusBreakdown.find((s: any) => s.type === 'PAID')?.count || 0, icon: CreditCard, color: "text-emerald-500", bg: "bg-emerald-500/10" },
        ...
        <div className="space-y-8 pb-20">
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
                <div className="space-y-4">
                    <h1 className="text-4xl md:text-5xl font-black text-slate-900 dark:text-white tracking-tight">
                        Student <span className="text-indigo-600 dark:text-indigo-500">Index</span>
                    </h1>
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-[repeat(auto-fill,minmax(320px,1fr))] gap-6">
                {isLoading ? (
                    ...
                    [
                        { name: "Total Students", count: pagination?.total || 0, type: 'TOTAL' },
                        ...planBreakdown
                                ...
                                <div className="flex justify-between items-start">
                                    <div>
                                        <p className="text-[10px] font-bold uppercase tracking-[0.2em] opacity-80">
                                            {plan.type === 'TOTAL' ? "User Demographics" : "Subscription Plan"}
                                        </p>
                                ...
                                <div className="flex justify-between items-end">
                                    <div>
                                        <p className="text-[10px] font-medium opacity-60">
                                            {plan.type === 'TOTAL' ? "Total Registered" : "Users Enrolled"}
                                        </p>
                    ...
                    <table className="w-full text-left border-collapse">
                        <thead>
                            <tr className="border-b border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-800/30">
                                ...
                                <th className="px-8 py-5 text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 dark:text-slate-500">Student Profile</th>
                                <th className="px-8 py-5 text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 dark:text-slate-500">Affiliation</th>
                            ...
                            {isLoading ? (
                                Array(5).fill(0).map((_, i) => (
                                    <tr key={i} className="animate-pulse">
                                        <td colSpan={5} className="px-8 py-6"><Skeleton className="h-12 w-full rounded-xl" /></td>
                                    </tr>
                                ))
                            ) : students?.length === 0 ? (
                                <tr>
                                    <td colSpan={5} className="px-8 py-32 text-center">
                                        <div className="flex flex-col items-center gap-4 opacity-30 grayscale">
                                            <StudentIcon size={64} className="text-slate-400" />
                                            <p className="text-xl font-bold text-slate-500">No students found</p>
                                        </div>
                                    </td>
                                </tr>
                            ) : (
                                students?.map((student: any, index: number) => (
                                    <tr 
                                        ...
                                        onClick={() => router.push(`/console/students/${student.id}`)}
                                    >
                                            ...
                                            <div className="flex items-center gap-4">
                                                ...
                                                <div className="flex flex-col min-w-0">
                                                    ...
                                                    <div className="flex items-center gap-3 text-xs font-bold text-slate-500">
                                                        <span className="text-indigo-500">{student.studentCode}</span>
