...
interface Props {
  activeTab: 'instructions' | 'attachments' | 'rubric' | 'materials' | 'quiz';
...
export default function TabNavigation({ activeTab, onTabChange, attachmentsCount = 0, questionsCount = 0 }: Props) {
  ...
  return (
    <div className="border-b border-slate-200 dark:border-slate-800 mb-6">
      <nav className="flex -mb-px overflow-x-auto gap-2" aria-label="Tabs">
        {tabs.map((tab) => (
          <button
            key={tab.id}
            ...
            className={`whitespace-nowrap border-b-2 px-4 py-4 text-sm font-medium transition-colors ${
              activeTab === tab.id
                ? 'border-primary text-primary dark:border-pink-500 dark:text-pink-400'
