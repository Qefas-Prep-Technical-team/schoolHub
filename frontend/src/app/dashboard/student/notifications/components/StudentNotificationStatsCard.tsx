...
interface StudentNotificationStatsCardProps {
  stats: NotificationStats;
}

export function StudentNotificationStatsCard({ stats }: StudentNotificationStatsCardProps) {
  const categories = [
    ...
    { label: 'Academic', count: stats.academic, icon: Activity, color: 'text-emerald-500', bg: 'bg-emerald-50 dark:bg-emerald-900/20' },
    { label: 'Messages', count: stats.messages, icon: Mail, color: 'text-pink-500', bg: 'bg-pink-50 dark:bg-pink-900/20' },
    { label: 'Assignments', count: stats.assignments, icon: Activity, color: 'text-purple-500', bg: 'bg-purple-50 dark:bg-purple-900/20' },
    ...
    <Card className="rounded-[2rem] border border-slate-200/50 dark:border-slate-800/50 shadow-sm bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl sticky top-8">
      <CardContent className="p-6">
        <div className="flex flex-col items-center mb-8">
          <div className="w-16 h-16 rounded-full bg-pink-500/20 flex items-center justify-center mb-4 relative">
            <Bell size={28} className="text-slate-900 dark:text-white" />
            {stats.unread > 0 && (
