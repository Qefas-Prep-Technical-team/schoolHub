...
const PerformanceChart: React.FC<PerformanceChartProps> = ({ 
  ...
  if (isLoading) {
    return (
      <div className="bg-white dark:bg-emerald-950/60 p-6 rounded-3xl border border-slate-100 dark:border-emerald-800/50 shadow-sm h-[382px] flex flex-col items-center justify-center">
        ...
        <div className="flex p-1.5 bg-gray-100 dark:bg-gray-800/50 rounded-2xl border border-gray-200 dark:border-gray-700">
          <button
            ...
            className={`px-5 py-2 text-xs font-bold rounded-xl transition-all duration-200 ${
              view === 'performance' 
                ? 'bg-white dark:bg-gray-700 text-emerald-600 shadow-[0_4px_12px_-2px_rgba(0,0,0,0.1)]' 
          ...
          <ResponsiveContainer width="100%" height="100%">
            {view === 'performance' ? (
              <BarChart data={performanceTrend} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                ...
                />
                <Tooltip 
                  cursor={{ fill: 'rgba(5, 150, 105, 0.05)', radius: 8 }}
                  contentStyle={{
