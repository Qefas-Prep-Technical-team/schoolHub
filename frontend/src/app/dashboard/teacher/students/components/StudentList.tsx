...
const StudentList: React.FC<StudentListProps> = ({ students, isLoading, error, searchQuery, currentPage = 1, itemsPerPage = 8 }) => {
  const getAttendanceStyles = (attendance: number) => {
    if (attendance < 75) return "text-red-600 dark:text-red-400 font-bold bg-red-50 dark:bg-red-900/10 px-2 py-0.5 rounded-md border border-red-100 dark:border-red-900/30";
    if (attendance < 90) return "text-amber-600 dark:text-amber-400 font-bold bg-amber-50 dark:bg-amber-900/10 px-2 py-0.5 rounded-md border border-amber-100 dark:border-amber-900/30";
    return "text-emerald-600 dark:text-emerald-400 font-bold bg-emerald-50 dark:bg-emerald-900/10 px-2 py-0.5 rounded-md border border-emerald-100 dark:border-emerald-900/30";
  };

  if (isLoading) {
    return (
      <div className="w-full bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden animate-pulse">
        <div className="h-12 bg-slate-50 dark:bg-slate-800/50 border-b border-slate-200 dark:border-slate-800" />
        {[1, 2, 3, 4, 5].map((i) => (
          <div key={i} className="flex h-20 items-center px-6 border-b border-slate-100 dark:border-slate-800/50">
  ...
  if (students.length === 0) {
    return (
      <motion.div 
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
        className="flex flex-col items-center justify-center py-20 rounded-2xl border border-dashed border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/10"
      >
                  ...
                  >
                    ...
                    <td className="px-6 py-4 text-right">
                      <Link href={`/dashboard/teacher/students/${student.id}`}>
                        <button className="inline-flex items-center justify-center gap-1.5 px-4 py-2 rounded-lg bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 text-xs font-bold hover:bg-emerald-500 hover:text-white dark:hover:bg-emerald-600 transition-all">
                          View Profile
                          <ArrowRight size={14} />
                        </button>
                      </Link>
