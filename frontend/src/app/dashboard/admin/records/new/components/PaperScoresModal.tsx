      ...
      >
        <div className="px-6 py-4 border-b border-slate-100 dark:border-slate-800/50 flex items-center justify-between flex-shrink-0">
          <h3 className="text-lg font-bold text-slate-900 dark:text-white flex items-center gap-2">
            <BookOpen className="w-5 h-5 text-[#5B5CE6]" />
        ...
        <div className="p-6 overflow-y-auto flex-1">
          {isLoading ? (
            <div className="flex flex-col items-center justify-center py-12 gap-3">
              <Loader2 className="w-8 h-8 animate-spin text-[#5B5CE6]" />
              ...
              <table className="w-full text-sm text-left">
                ...
                <tbody className="divide-y divide-slate-100 dark:divide-slate-800/50">
                  {grades.map((grade: any, index: number) => {
                    ...
                    return (
                      <tr key={grade.id} className={`transition-colors ${isCurrent ? "bg-[#5B5CE6]/5 hover:bg-[#5B5CE6]/10" : "hover:bg-slate-50/50 dark:hover:bg-slate-900/20"}`}>
                        ...
                        <td className="px-4 py-3 font-medium text-slate-900 dark:text-slate-100 flex items-center gap-2">
                          {studentName}
                          {isCurrent && (
                            <span className="text-[10px] uppercase font-bold tracking-wider bg-[#5B5CE6]/10 text-[#5B5CE6] px-1.5 py-0.5 rounded flex-shrink-0">In Result</span>
                          )}
                        </td>
                        <td className="px-4 py-3 text-slate-500">
                          {studentCode}
                        </td>
                        <td className="px-4 py-3 font-semibold text-[#5B5CE6] text-right">
                          {grade.score ?? "-"}
                        </td>
                      </tr>
                    );
