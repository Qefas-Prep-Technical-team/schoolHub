...
interface AttendanceSwipeModalProps {
  isOpen: boolean;
  onClose: () => void;
...
const AttendanceSwipeModal: React.FC<AttendanceSwipeModalProps> = ({
  isOpen,
  onClose,
  onSave,
  students,
    ...
    if (isOpen) {
      const records = students.map(student => {
        ...
        return {
          ...
          className: existing?.className || 'Class',
          date: date,
          status: (existing?.status as AttendanceStatus) || 'present',
          comment: existing?.comment || (existing as any)?.note || '',
          submittedBy: existing?.submittedBy || 'Teacher',
          submittedAt: existing?.submittedAt || new Date().toISOString()
        };
      });
      // Start with all "present" as a fallback, but we will overwrite them when they swipe.
      setAttendanceRecords(records);
      setCurrentIndex(0);
      setShowSummary(students.length === 0);
    }
        ...
        <div className="flex-1 relative flex flex-col items-center justify-center p-6 bg-slate-50 dark:bg-slate-950 overflow-hidden">
          ...
          {showSummary && (
            <div className="w-full animate-in fade-in slide-in-from-bottom-4 duration-300 flex flex-col h-full">
              ...
              <div className="flex-1 overflow-y-auto w-full bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-4 space-y-3 max-h-[300px]">
                {attendanceRecords.map(record => (
                  <div key={record.studentId} className="flex items-center justify-between p-3 rounded-xl bg-slate-50 dark:bg-slate-800/50">
                    <div>
                      <p className="font-bold text-sm text-slate-900 dark:text-white">{record.studentName}</p>
                      <p className="text-xs text-slate-500 font-mono">{record.studentCode}</p>
                    </div>
...
const SwipeCard = ({ 
  ...
  return (
    ...
    >
      {/* Dynamic Overlays */}
      <motion.div style={{ opacity: presentOpacity }} className="absolute inset-0 bg-green-500/20 z-10 flex items-center justify-center">
...
export default AttendanceSwipeModal;
