...
const ImageUpload: React.FC<ImageUploadProps> = ({ 
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const { uploadUrl, publicUrl } = await imageService.getCloudflareUploadUrl(file.type);
      
      // 2. Upload directly to Cloudflare R2
      await imageService.uploadToR2(uploadUrl, file);
      
      // 3. Callback to update parent state with the public URL
      onChange(publicUrl);
      toast.success(`${label} uploaded successfully!`);
    } catch (error: any) {
      console.error('Upload failed:', error);
      toast.error(error.message || 'Failed to upload image. Check your Cloudflare credentials.');
    } finally {
