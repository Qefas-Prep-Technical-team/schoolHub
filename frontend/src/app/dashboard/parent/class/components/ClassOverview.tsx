      ...
      <div className="space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="col-span-1 md:col-span-2 space-y-6">
            <div className="bg-white dark:bg-slate-900 p-6 rounded-[20px] border border-slate-100 dark:border-slate-800 shadow-sm">
              <div className="flex items-center gap-3 mb-6">
                ...
                <div className="h-5 w-1/3 bg-slate-100 dark:bg-slate-800 rounded-lg animate-pulse" />
              </div>
              <div className="h-64 w-full bg-slate-50 dark:bg-slate-800/50 rounded-xl animate-pulse" />
            </div>
            
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              {[1, 2].map((i) => (
                <div key={i} className="bg-white dark:bg-slate-900 p-6 rounded-[20px] border border-slate-100 dark:border-slate-800 shadow-sm flex flex-col items-center justify-center space-y-4">
                   ...
                   <div className="h-6 w-1/3 bg-slate-100 dark:bg-slate-800 rounded-lg animate-pulse" />
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="col-span-1 space-y-6">
            <div className="bg-white dark:bg-slate-900 p-6 rounded-[20px] border border-slate-100 dark:border-slate-800 shadow-sm space-y-6">
              ...
              <div className="space-y-4">
                {[1, 2, 3].map((i) => (
                  <div key={i} className="p-4 rounded-xl bg-slate-50 dark:bg-slate-800/50 flex gap-4 items-center">
                    <div className="w-12 h-12 rounded-xl bg-slate-100 dark:bg-slate-700 animate-pulse shrink-0" />
    ...
    <div className="min-h-screen bg-background-light dark:bg-background-dark font-display text-slate-900 dark:text-slate-100 transition-colors duration-200">
      <main className="flex-1 w-full px-4 md:px-0 py-8">
        <div className="w-[95%] mx-auto flex flex-col gap-8">
                  ...
                  <>
                    <span className="flex items-center gap-1.5">
                      <span className="w-2 h-2 rounded-full bg-orange-600 animate-pulse" />
                       Code: <span className="text-slate-900 dark:text-white font-semibold">{classData?.classCode}</span>
                    </span>
                    {classData?.level && (
                      <>
                        <span className="text-slate-300 dark:text-slate-700">|</span>
                        <span>
                          Level: <span className="text-slate-900 dark:text-white font-semibold">{classData.level}</span>
                        </span>
                      </>
                    )}
                    {classData?.session && (
                      <>
                        <span className="text-slate-300 dark:text-slate-700">|</span>
                        <span>
                          Session: <span className="text-slate-900 dark:text-white font-semibold">{classData.session}</span>
                        </span>
                      </>
                    )}
                    {classData?.term && (
                      <>
                        <span className="text-slate-300 dark:text-slate-700">|</span>
                        <span>
                          Term: <span className="text-slate-900 dark:text-white font-semibold">{classData.term}</span>
                        </span>
                      </>
                    )}
                  </>
          ...
          {/* Tabs */}
          <div className="bg-white dark:bg-slate-900 rounded-[20px] shadow-sm border border-slate-100 dark:border-slate-800 p-2 overflow-hidden">
            <CustomTabs
              tabs={tabs}
              activeTab={activeTab}
