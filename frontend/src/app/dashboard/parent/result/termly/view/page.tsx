...
import { useChildResults } from "@/lib/api/hooks/useParentChildren";
import { Skeleton } from "@/components/ui/skeleton";
import { useSearchParams, useRouter } from "next/navigation";
import { Suspense } from "react";
import { ChevronLeft, BookOpen, GraduationCap, Lock, TrendingUp, Award, BarChart3, CheckCircle2, Eye } from "lucide-react";
...
function NACell() {
  return <span className="inline-flex items-center gap-1 text-slate-400 dark:text-slate-600 text-xs italic"><Lock className="w-3 h-3" />N/A</span>;
}
...
function ResultViewInner() {
  ...
  const { data: results = [], isLoading } = useChildResults(childId || null);
  ...
  const group = groups.find((g: any) => g.sessionId === sessionId && g.term === term);

  if (!group) {
    return (
      <div className="w-[95%] mx-auto py-8">
        <button onClick={() => router.back()} className="flex items-center gap-2 text-sm text-slate-500 hover:text-slate-800 dark:hover:text-slate-200 transition-colors mb-8">
        ...
        <div className="flex flex-col items-center justify-center py-24 rounded-3xl border-2 border-dashed border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-900/50">
          ...
          <p className="text-slate-400 text-sm">This result is not available yet, or your child does not have access to it.</p>
        </div>
      </div>
    );
  }
  ...
  const subjectResults = group.subjectResults ?? [];
  ...
  return (
    <div className="w-[95%] mx-auto py-8">
      <button onClick={() => router.back()} className="flex items-center gap-2 text-sm text-slate-500 hover:text-slate-800 dark:hover:text-slate-200 transition-colors mb-6 group">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div className="flex items-center gap-4">
            ...
            <div>
              <div className="flex items-center gap-2 flex-wrap">
                <span className="text-xs font-bold uppercase tracking-widest opacity-80">{getTermLabel(group.term)}</span>
                <span className="opacity-40">•</span>
                <span className="text-xs font-medium opacity-80">{group.session?.name || "—"}</span>
              </div>
              <h1 className="text-2xl md:text-3xl font-black mt-1 leading-tight">{group.class?.name || "Class"}</h1>
          ...
          {allReleased && avg != null ? (
            ...
            <div className="bg-white/20 rounded-xl px-5 py-4 flex items-center gap-3 max-w-xs">
              ...
              <p className="text-sm font-medium">Subjects are visible but scores are not yet released by the school.</p>
            </div>
      ...
      {noneReleased && (
        <div className="mb-6 flex items-start gap-3 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800/50 rounded-2xl px-5 py-4 text-amber-800 dark:text-amber-300">
          ...
          <div>
            ...
            <p className="text-sm opacity-80 mt-0.5">Subjects are visible but scores are still being finalised. Check back later.</p>
          </div>
        </div>
      )}
      {someReleased && (
        <div className="mb-6 flex items-start gap-3 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800/50 rounded-2xl px-5 py-4 text-blue-800 dark:text-blue-300">
          ...
          <div>
            ...
            <p className="text-sm opacity-80 mt-0.5">{revealedCount} of {subjectCount} subject score{revealedCount !== 1 ? "s" : ""} have been released. Remaining scores show N/A until the school releases them.</p>
          </div>
        </div>
      )}
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800/60">
              {subjectResults.map((subject: any) => {
                const released = subject.scoresRevealed;
                const total = released && subject.totalScore != null ? Number(subject.totalScore) : null;
                return (
                  <tr key={subject.id} className="hover:bg-slate-50/60 dark:hover:bg-slate-800/30 transition-colors">
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        ...
                        <span className="font-semibold text-slate-800 dark:text-slate-200">{subject.subject?.name || "Subject"}</span>
                        {!released && <span className="text-[10px] uppercase font-bold tracking-wider text-amber-500 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800/30 px-2 py-0.5 rounded-full">Pending</span>}
                      </div>
                    </td>
                    <td className="px-6 py-4 text-center text-slate-600 dark:text-slate-400">{released ? (subject.assignmentScore != null ? subject.assignmentScore : "—") : <NACell />}</td>
                    <td className="px-6 py-4 text-center text-slate-600 dark:text-slate-400">{released ? ((subject.caScore ?? subject.quizScore) != null ? (subject.caScore ?? subject.quizScore) : "—") : <NACell />}</td>
                    <td className="px-6 py-4 text-center text-slate-600 dark:text-slate-400">{released ? (subject.examScore != null ? subject.examScore : "—") : <NACell />}</td>
                    <td className="px-6 py-4 text-center">
                      {released ? (
                        <span className={`inline-flex items-center justify-center min-w-[3.5rem] px-3 py-1.5 rounded-lg text-sm font-bold ${getScoreColor(total)}`}>{total != null ? total : "—"}</span>
                      ) : <NACell />}
                    </td>
                    <td className="px-6 py-4 text-center">
                      {released ? (
                        <span className={`inline-flex items-center justify-center w-9 h-9 rounded-full text-sm font-black ${getScoreColor(total)}`}>{getGradeLetter(total)}</span>
              ...
              {subjectResults.length === 0 && (
                <tr><td colSpan={6} className="px-6 py-14 text-center text-slate-400 italic">No subjects available yet.</td></tr>
              )}
            </tbody>
          </table>
        </div>
          ...
          <div className="px-6 py-5 bg-slate-50 dark:bg-slate-900/50 border-t border-slate-100 dark:border-slate-800">
            <div className="flex flex-wrap gap-6">
              <div className="flex items-center gap-2 text-sm">
                ...
                <span className="font-bold text-slate-800 dark:text-slate-200">{avg != null ? avg.toFixed(1) + "%" : "—"}</span>
              </div>
              <div className="flex items-center gap-2 text-sm">
              ...
              <div className="flex items-center gap-2 text-sm">
                ...
                <span className="font-bold text-slate-800 dark:text-slate-200">{revealedCount} / {subjectCount}</span>
              </div>
            </div>
          </div>
        )}
...
export default function ParentResultViewPage() {
  return (
    <Suspense fallback={<div className="w-[95%] mx-auto py-8 space-y-5"><Skeleton className="h-10 w-48 rounded-xl" /><Skeleton className="h-40 w-full rounded-2xl" /><Skeleton className="h-64 w-full rounded-2xl" /></div>}>
      <ResultViewInner />
    </Suspense>
  );
}
