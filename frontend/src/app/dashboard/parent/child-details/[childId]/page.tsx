...
import {
    ...
    Trash2, Plus, UserMinus, FileText
} from 'lucide-react'
...
export default function StudentProfilePage() {
    ...
    const childId = typeof params.childId === 'string' ? params.childId : Array.isArray(params.childId) ? params.childId[0] : ''

    // ── Queries ──────────────────────────────────────────────────────────────
    const { data: student, isLoading: isStudentLoading, error: studentError } = useQuery({
        queryKey: ['student-details', childId],
        queryFn: () => studentService.getStudentById(childId),
        enabled: !!childId
    })

    const { data: gradesData, isLoading: isGradesLoading } = useQuery({
        queryKey: ['student-grades', childId],
        queryFn: () => gradeService.getStudentGrades(childId, { limit: 100 }),
        enabled: !!childId
    })

    const { data: historyData, isLoading: isHistoryLoading } = useQuery({
        queryKey: ['student-history', childId],
        queryFn: () => studentService.getStudentHistory(childId),
        enabled: !!childId
    })

    const { data: assignmentsData, isLoading: isAssignmentsLoading } = useQuery({
        queryKey: ['student-assignments', childId],
        queryFn: () => studentService.getStudentAssignments(childId),
        enabled: !!childId,
        staleTime: 1000 * 60 * 2, // 2 mins
    })

    const { data: attendanceRecords } = useStudentAttendance(childId);

    const computedAttendanceRate = useMemo(() => {
    ...
    const primaryColor = settings?.themeColor || '#ea580c' // default to orange
    ...
    const { data: behaviourProfile, isLoading: isBehaviourProfileLoading } = useStudentBehaviourProfile(childId)
    const { data: behaviourAlerts, isLoading: isAlertsLoading } = useClassBehaviourAlerts(classId, childId)

    const updateBehaviourProfileMutation = useUpdateStudentBehaviourProfile(childId)
        ...
        <div className="min-h-screen bg-slate-100 dark:bg-slate-950 transition-colors duration-500">
                    ...
                    <div className="lg:col-span-2 space-y-6 md:space-y-10">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                            ...
                            <StatBadge 
                                ...
                                icon={Activity} 
                                themeColor="#10b981" 
                                trend={computedAttendanceRate >= 90 ? "Excellent" : computedAttendanceRate >= 75 ? "Good" : "Needs Attention"} 
                            />
                        </div>
            ...
            {activeTab === 'attendance' && (
                <main className="max-w-7xl mx-auto px-0 md:px-12 mt-10 pb-20">
                     <AttendanceCalendar childId={childId} themeColor={primaryColor} />
                </main>
            )}
            ...
            {/* ── Teachers Tab ─────────────────────────────────────────────── */}
            {activeTab === 'teachers' && (
                <main className="max-w-7xl mx-auto px-4 md:px-12 mt-10 pb-20">
                    <SectionCard title="Class Teachers">
                        {student.classes && student.classes.length > 0 && student.classes[0].class.teachers && student.classes[0].class.teachers.length > 0 ? (
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                                {student.classes[0].class.teachers.map((ct: any) => {
                                    const teacher = ct.teacher;
                                    return (
                                        <div key={teacher.id} className="bg-slate-50 dark:bg-white/[0.02] border border-slate-100 dark:border-white/5 rounded-3xl p-6 flex flex-col items-center text-center hover:border-primary/30 transition-all shadow-sm hover:shadow-xl hover:-translate-y-1">
                                            {teacher.bannerImage ? (
                                                <img src={teacher.bannerImage} alt={teacher.name} className="size-24 rounded-[2rem] mb-4 object-cover border-4 border-white dark:border-slate-800 shadow-md" />
                                            ) : (
                                                <div className="size-24 rounded-[2rem] mb-4 bg-primary/10 flex items-center justify-center border-4 border-white dark:border-slate-800 shadow-md">
                                                    <User size={40} className="text-primary" style={{ color: primaryColor }} />
                                                </div>
                                            )}
                                            
                                            <h3 className="text-lg font-black text-slate-900 dark:text-white uppercase tracking-tight mb-1">{teacher.name}</h3>
                                            <p className="text-[10px] font-black text-primary uppercase tracking-[0.2em] mb-4" style={{ color: primaryColor }}>
                                                {ct.isLead ? 'Class Teacher' : 'Subject Teacher'}
                                            </p>
                                            ...
                                            {teacher.email && (
                                                <a href={`mailto:${teacher.email}`} className="w-full py-3 rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-300 font-bold text-sm flex items-center justify-center gap-2 hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors shadow-sm">
                            ...
                            <div className="py-20 flex flex-col items-center justify-center text-center space-y-4">
                                ...
                                <h3 className="text-xl font-bold text-slate-900 dark:text-white">No Teachers Assigned</h3>
                                <p className="text-slate-500 max-w-sm mx-auto">There are no teachers assigned to this student's class yet.</p>
                            </div>
                        )}
                    </SectionCard>
                </main>
            ...
            {activeTab === 'assignments' && (
                <main className="max-w-7xl mx-auto px-4 md:px-12 mt-10 pb-20">
                    <SectionCard title="Student Assignments">
                        {isAssignmentsLoading ? (
                                        ...
                                        <div key={assignment.id} onClick={() => router.push(`/dashboard/parent/assignments/details/${assignment.id}`)} className="bg-slate-50 dark:bg-white/[0.02] border border-slate-100 dark:border-white/5 rounded-3xl p-6 flex flex-col hover:border-primary/30 transition-all shadow-sm hover:shadow-xl hover:-translate-y-1 cursor-pointer">
                                            <div className="flex items-start justify-between mb-4">
                                                ...
                                                <span className={cn(
                                                    ...
                                                    status === 'GRADED' || status === 'SCORED' ? "bg-emerald-50 text-emerald-600 border-emerald-100 dark:bg-emerald-500/10 dark:text-emerald-400 dark:border-emerald-500/20" :
                                                    status === 'SUBMITTED' ? "bg-blue-50 text-blue-600 border-blue-100 dark:bg-blue-500/10 dark:text-blue-400 dark:border-blue-500/20" :
                                                    "bg-amber-50 text-amber-600 border-amber-100 dark:bg-amber-500/10 dark:text-amber-400 dark:border-amber-500/20"
                                                )}>
                                                    {status}
                                                </span>
                                            ...
                                            <h3 className="text-lg font-black text-slate-900 dark:text-white tracking-tight mb-2 line-clamp-2">{assignment.title}</h3>
                                            
                                            {assignment.subject && (
                                                <div className="flex items-center gap-2 text-xs text-slate-600 dark:text-slate-400 mb-4 font-bold">
                                            ...
                                            <div className="mt-auto pt-4 border-t border-slate-200 dark:border-white/10 flex items-center justify-between">
                                                ...
                                                {(status === 'GRADED' || status === 'SCORED') && assignment.grade && (
                                                    <div className="text-right space-y-1">
                                                        <p className="text-[9px] font-black text-emerald-500 uppercase tracking-widest">Score</p>
                                                        <p className="text-lg font-black text-emerald-600 dark:text-emerald-400 leading-none">
                                                            {assignment.grade.split('/')[0]}
                                                            <span className="text-xs text-emerald-500/50">/{assignment.grade.split('/')[1] || assignment.totalMarks || 100}</span>
                                                        </p>
                                                    </div>
                                                )}
                                            </div>
                            ...
                            <div className="py-20 flex flex-col items-center justify-center text-center space-y-4">
                                ...
                                <p className="text-slate-500 max-w-sm mx-auto">This student currently has no assignments.</p>
                            </div>
                        )}
                    </SectionCard>
                </main>
